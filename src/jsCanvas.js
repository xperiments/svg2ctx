...
(function ()
{
  ...
  /* Edit History:
   * 03/05/2013 xperiments Addapted Code to work with FabricJS & Node
   */
...
//
  ...
  this.jsCanvas = function(objectname, width, height) {
...
// all of these are private objects/methods.
  ...
  function jsContext2d (canvas, objectname) {
    ...
    this.fn = objectname;
    this.output = '';
    this.output += 'window.'+this.fn+'= function (ctx) { this.ctx = ctx; this.initGradients();};\n';
    this.output += 'window.'+this.fn+'.prototype.setter = function (ctx, key, val) { ctx [key] = val; };\n';
    this.output += 'window.'+this.fn+'.prototype.render = function (ctx)\n{\n\tif (! ctx) ctx = this.ctx; \n';
    this.outputimages = 'window.'+this.fn+'.imgs = [];\n';
    this.outputimages += 'window.'+this.fn+'.loadImages = function () { \n';
    this.outputGradients = 'window.'+this.fn+'.prototype.initGradients = function ()\n{\n\tthis.gradients = []; \n';
    this.ngradients = 0;
    this.nimages = 0;
  }
  ...
  jsContext2d.prototype.done = function () {
    if (! this.finished) {
      ...
      this.output += this.outputGradients;
...
jsContext2d.prototype.emitGradient = function (fn, args, fnprefix) {
  ...
  jsContext2d.prototype.createLinearGradient = function () {
    ...
    g.addColorStop = function (stop, color) {
      state.outputGradients += sprintf ('\tthis.gradients [%d].addColorStop (%f, "%s");\n',
        n, stop, color);
      // execute in parent
